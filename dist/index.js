...
class Binliner extends Array {
  ...
  setValidation = (validation) => {
    if (Array.isArray(validation) && validation.some((v) => !["string", "number"].includes(typeof v))) {
      throw new Error("Array validation must contain only strings or numbers.");
    } else if (!Array.isArray(validation) && !["string", "number", "function"].includes(typeof validation)) {
